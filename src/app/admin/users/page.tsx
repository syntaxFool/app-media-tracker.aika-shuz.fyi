  ...
  return (
    <AppLayout>
      <div className="p-4 max-w-3xl mx-auto space-y-4">
        <div className="flex items-center justify-between"><h1 className="text-heading-3 text-fg-primary">Users</h1><button onClick={()=>setShowForm(!showForm)} className="btn-primary flex items-center gap-1.5"><Plus className="w-4 h-4"/>Add User</button></div>

        {showForm&&<form onSubmit={handleCreate} className="bg-white border border-border rounded-md p-4 space-y-3 shadow-sm animate-fade-in">
          <div><label className="block text-label text-fg-tertiary mb-1">Username</label><input type="text" value={form.username} onChange={e=>setForm({...form,username:e.target.value})} className="input-linear w-full" required autoFocus/></div>
          ...
          <div className="flex gap-2"><button type="submit" disabled={submitting} className="btn-primary flex-1">{submitting?"Creating...":"Create User"}</button><button type="button" onClick={()=>setShowForm(false)} className="btn-ghost">Cancel</button></div>
        </form>}

        {loading?<div className="flex justify-center py-8"><Loader2 className="w-6 h-6 text-fg-tertiary animate-spin"/></div>
        :<div className="bg-white border border-border rounded-md overflow-hidden shadow-sm"><table className="table-linear"><thead><tr><th>Username</th><th>Role</th><th>Created</th></tr></thead><tbody>{users.map(user=><tr key={user.id}><td className="font-[510] text-fg-primary">{user.username}</td><td><span className={`badge ${user.role==="admin"?"badge-success":"badge-neutral"}`}>{user.role}</span></td><td>{new Date(user.createdAt).toLocaleDateString("en-IN")}</td></tr>)}</tbody></table></div>}
      </div>
    </AppLayout>
  );
}
