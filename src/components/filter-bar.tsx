...
export default function FilterBar({
  ...
  return (
    <div className="flex gap-2 overflow-x-auto pb-2 px-4 -mx-4 scrollbar-hide">
      {/* Influencer toggle */}
      <div className="flex items-center gap-1 bg-white/[0.03] border border-white/[0.06] rounded-sm px-2 py-1.5 flex-shrink-0">
        <StarToggle
          active={influencerFilter === "true"}
          onClick={() =>
            onInfluencerChange(influencerFilter === "true" ? "" : "true")
          }
        />
      </div>
...
function StarToggle({ active, onClick }: { active: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1 text-label transition-all ${
        active ? "text-yellow-400" : "text-fg-quaternary"
      }`}
    >
      <svg
        className="w-4 h-4"
